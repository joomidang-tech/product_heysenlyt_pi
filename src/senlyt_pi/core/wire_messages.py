...
# RecipeStep.kind 종류(§9-1 v2 — 2026-07-14 병렬 stage 계약 · v3 batchSyringe 2026-07-21).
...
# 배치 흡입(§9-1 v3 · 2026-07-21 "1향료 1펌핑" QA 봉합) — v1.1.0 `_dispenseCurrentBatch` 복원.
#   같은 펌프의 여러 액체를 밸브 돌려가며 한 주사기에 **순서대로 누적 흡입 → 한 번 배출**.
#   흡입은 절대이동 누적(A{합})이라 합이 시린지 용량을 넘으면 Code 11 과충전 — 서버·pi 양측이
#   누적합 ≤ 시린지 용량을 검증한다. 정본 계약 = heysenlyt-web `lib/server/wire.ts` BatchSyringeStep.
BATCH_SYRINGE_STEP_KIND = "batchSyringe"
...
class BatchAspiration:
    """배치 흡입의 한 액체(§9-1 v3) — 정본 = wire.ts `BatchSyringeStep.aspirations[number]`.

    한 주사기에 누적 흡입할 액체 1종. 흡입 순서 = tuple 순서 = 실행 순서(누적 절대이동).
    """

    flavor: str
    in_port: int  # `I{in_port}` — 이 액체가 꽂힌 흡입 구멍
    volume: float  # µL — 이 액체의 흡입 부피(합 ≤ 시린지 용량·과충전 방지)
    aspirate_speed_hz: int  # 이 구멍 흡입 속도(서버가 전역×오버라이드 해석)
...
class RecipeStep:
    ...
    def is_batch(self) -> bool:
        return self.kind == BATCH_SYRINGE_STEP_KIND
    ...
    def from_json(j: Mapping[str, Any]) -> "RecipeStep":
        ...
        if kind == BATCH_SYRINGE_STEP_KIND:
            # 배치 흡입(§9-1 v3) — 정본 = wire.ts BatchSyringeStep. 흡입 순서 보존(누적 절대이동).
            #   outPort/dispenseSpeedHz/slope 는 배치 공유값(RecipeStep 의 out_port/dispense_speed_hz/
            #   slope 필드 재사용), aspirations 는 액체별 흡입 목록.
            raw_asps = j.get("aspirations")
            aspirations = tuple(
                BatchAspiration(
                    flavor=str(a.get("flavor", "")),
                    in_port=int(a["inPort"]),
                    volume=float(a["volume"]),
                    aspirate_speed_hz=int(a["aspirateSpeedHz"]),
                )
                for a in raw_asps
                if isinstance(a, Mapping)
            ) if isinstance(raw_asps, Sequence) and not isinstance(raw_asps, (str, bytes)) else ()
            label = ",".join(a.flavor for a in aspirations if a.flavor) or "batch"
            return RecipeStep(
                idx=int(j["idx"]),
                pump_addr=int(j["pumpAddr"]),
                flavor=str(j.get("flavor") or label),
                volume=0.0,  # 배치 부피는 aspiration 별 — 상위 volume 은 미사용.
                kind=BATCH_SYRINGE_STEP_KIND,
                stage=stage,
                out_port=int(j["outPort"]),
                dispense_speed_hz=int(j["dispenseSpeedHz"]),
                slope=int(j["slope"]),
                ...
                "aspirations": [
                    {
                        ...
                        "inPort": a.in_port,
                        "volume": a.volume,
                        "aspirateSpeedHz": a.aspirate_speed_hz,
                    }
                    for a in (self.aspirations or ())
                ],
            }
