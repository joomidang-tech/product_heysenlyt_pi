...
이 계층 고유 책임:
  ...
  - **recipe 해석**: command.recipe is None(§9-1 폴백)이면 recipeId(flavor)/fragranceResult
    (fragrance)/**expoRecipe**(v1.2.0 식향 생성형)로 RecipeStep 리스트를 만든다.
    fragrance/expo 는 mL→µL 정규화(§6-6·Code 11 방지) — 헬퍼는
    pipeline.recipe_resolver(expo_recipe_to_steps·flavor_recipe_source_to_steps)와 이 모듈
...
# recipe==None 폴백 해석기 — recipeId/fragranceResult/expoRecipe → RecipeStep 리스트.
#
# 실제 레시피 소스(flavor_recipes·expoRecipe payload·fragranceResult.notes)는 pi settings/
# 명령 payload 에서 온다. 이 Callable 은 그 해석을 주입 가능하게 하여(테스트 결정성)
# dispatcher 를 순수하게 유지한다. 반환 리스트는 아직 검증 전(RR 이 게이트) — 단,
# fragrance/expo 는 여기서 mL→µL 정규화 완료(§6-6).
...
class Dispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        # recipe==None 폴백 해석(recipeId/fragranceResult/expoRecipe). recipe 있으면 그 steps 사용.
    ...
    def _on_command(self, command: Command) -> JobReport | None:
        ...
        # recipe 해석: 명시 steps 우선, None 이면 폴백 해석(recipeId/fragranceResult/expo·mL→µL).
