...
def test_pump_map_from_settings_builtin_preset():
    """빌트인 sy01b — fullStroke 12000 강제, flavor 1.25mL → maxVolumeUl 1250."""
    settings = {
        "pumps": [
...
def test_capacity_allowlist_fallback_to_mode_default():
    """9종 allowlist 밖 용량 → 모드 기본값 폴백(flavor 1.25 / fragrance 0.5·O-15)."""
    settings = {
        "pumps": [
    ...
    m = pump_map_from_settings(settings)
    assert m[1].syringe_capacity_ml == 1.25
